...
const Alojamiento = () => {
  ...
  useEffect(() => {
    const cargarEventos = async () => {
      try {
        setLoading(true);
        console.log('Iniciando carga de alojamientos...');
        const eventosAlojamiento = await getEventosPorCategoria("Alojamiento");
        console.log('Respuesta del servidor:', eventosAlojamiento);
        
        if (!eventosAlojamiento) {
          console.warn('No se recibieron eventos del servidor');
          setEventos([]);
          return;
        }

        if (!Array.isArray(eventosAlojamiento)) {
          console.error('La respuesta no es un array:', eventosAlojamiento);
          setError('Error en el formato de los datos recibidos');
          return;
        }

        console.log('Alojamientos cargados:', eventosAlojamiento);
        setEventos(eventosAlojamiento);
      } catch (error) {
    ...
    <div className="app-container">
      ...
      <main className="seccion-container">
        <div className="seccion-header">
          <h1 className="seccion-titulo">Alojamiento</h1>
          <p className="seccion-descripcion">Descubre los mejores lugares para alojarte</p>
        </div>
        ...
        <div className="eventos-grid">
          {eventos && eventos.length > 0 ? (
            ...
            <div className="no-eventos">
              ...
              <p className="no-eventos-sub">¡Vuelve pronto para ver nuevos alojamientos!</p>
            </div>
          )}
        </div>
