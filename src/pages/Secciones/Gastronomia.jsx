...
const Gastronomia = () => {
  ...
  useEffect(() => {
    const cargarEventos = async () => {
      try {
        setLoading(true);
        console.log('Iniciando carga de gastronomía...');
        const eventosGastronomia = await getEventosPorCategoria("Gastronomia");
        console.log('Respuesta del servidor:', eventosGastronomia);
        
        if (!eventosGastronomia) {
          console.warn('No se recibieron eventos del servidor');
          setEventos([]);
          return;
        }

        if (!Array.isArray(eventosGastronomia)) {
          console.error('La respuesta no es un array:', eventosGastronomia);
          setError('Error en el formato de los datos recibidos');
          return;
        }

        console.log('Gastronomía cargada:', eventosGastronomia);
    ...
    <div className="app-container">
      ...
      <main className="seccion-container">
        <div className="seccion-header">
          <h1 className="seccion-titulo">Gastronomía</h1>
          <p className="seccion-descripcion">Descubre los mejores restaurantes y experiencias culinarias</p>
        </div>
        ...
        <div className="eventos-grid">
          {eventos && eventos.length > 0 ? (
            ...
            <div className="no-eventos">
              <p>No hay eventos gastronómicos disponibles en este momento</p>
              <p className="no-eventos-sub">¡Vuelve pronto para ver nuevos eventos gastronómicos!</p>
            </div>
          )}
        </div>
