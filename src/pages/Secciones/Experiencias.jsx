  ...
  useEffect(() => {
    const cargarEventos = async () => {
      try {
        ...
        const eventosExperiencias = await getEventosPorCategoria("Experiencias");
        console.log('Respuesta del servidor:', eventosExperiencias);
        
        if (!eventosExperiencias) {
          console.warn('No se recibieron eventos del servidor');
          setEventos([]);
          return;
        }

        if (!Array.isArray(eventosExperiencias)) {
          console.error('La respuesta no es un array:', eventosExperiencias);
          setError('Error en el formato de los datos recibidos');
          return;
        }

        console.log('Experiencias cargadas:', eventosExperiencias);
        setEventos(eventosExperiencias);
      } catch (error) {
    ...
    <div className="app-container">
      ...
      <main className="seccion-container">
        <div className="seccion-header">
          <h1 className="seccion-titulo">Experiencias</h1>
          <p className="seccion-descripcion">Descubre las mejores experiencias y actividades</p>
        </div>
        ...
        <div className="eventos-grid">
          {eventos && eventos.length > 0 ? (
            ...
            <div className="no-eventos">
              ...
              <p className="no-eventos-sub">¡Vuelve pronto para ver nuevas experiencias!</p>
            </div>
          )}
        </div>
