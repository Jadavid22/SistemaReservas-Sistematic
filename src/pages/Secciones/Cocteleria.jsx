...
const Cocteleria = () => {
  ...
  useEffect(() => {
    const cargarEventos = async () => {
      try {
        setLoading(true);
        console.log('Iniciando carga de eventos de coctelería...');
        const eventosCocteleria = await getEventosPorCategoria("Cocteleria");
        console.log('Respuesta del servidor:', eventosCocteleria);
        
        if (!eventosCocteleria) {
          console.warn('No se recibieron eventos del servidor');
          setEventos([]);
          return;
        }

        if (!Array.isArray(eventosCocteleria)) {
          console.error('La respuesta no es un array:', eventosCocteleria);
          setError('Error en el formato de los datos recibidos');
          return;
        }

        console.log('Eventos de coctelería cargados:', eventosCocteleria);
        setEventos(eventosCocteleria);
      } catch (error) {
    ...
    return (
      <div className="app-container">
        ...
        <div className="loading-container">
          ...
          <p>Cargando eventos de coctelería...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="app-container">
      ...
      <main className="seccion-container">
        <div className="seccion-header">
          <h1 className="seccion-titulo">Coctelería</h1>
          <p className="seccion-descripcion">Descubre los mejores bares y experiencias de coctelería</p>
        </div>
        ...
        <div className="eventos-grid">
          {eventos && eventos.length > 0 ? (
            ...
            <div className="no-eventos">
              <p>No hay eventos de coctelería disponibles en este momento</p>
