...
const Arte = () => {
  ...
  useEffect(() => {
    const cargarEventos = async () => {
      try {
        setLoading(true);
        console.log('Iniciando carga de eventos de arte...');
        const eventosArte = await getEventosPorCategoria("Arte");
        console.log('Respuesta del servidor:', eventosArte);
        ...
        if (!Array.isArray(eventosArte)) {
          console.error('La respuesta no es un array:', eventosArte);
          setError('Error en el formato de los datos recibidos');
          return;
        }

        console.log('Eventos de arte cargados:', eventosArte);
        setEventos(eventosArte);
      } catch (error) {
    ...
    <div className="app-container">
      ...
      <main className="seccion-container">
        <div className="seccion-header">
          <h1 className="seccion-titulo">Arte</h1>
          <p className="seccion-descripcion">Descubre los mejores eventos de arte y experiencias</p>
        </div>
        ...
        <div className="eventos-grid">
          {eventos && eventos.length > 0 ? (
            ...
            <div className="no-eventos">
              <p>No hay eventos de arte disponibles en este momento</p>
              <p className="no-eventos-sub">¡Vuelve pronto para ver nuevos eventos de arte!</p>
            </div>
          )}
        </div>
