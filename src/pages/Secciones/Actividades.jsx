  ...
  useEffect(() => {
    const cargarEventos = async () => {
      try {
        ...
        const eventosActividades = await getEventosPorCategoria("Actividades");
        console.log('Respuesta del servidor:', eventosActividades);
        
        if (!eventosActividades) {
          console.warn('No se recibieron eventos del servidor');
          setEventos([]);
          return;
        }

        if (!Array.isArray(eventosActividades)) {
          console.error('La respuesta no es un array:', eventosActividades);
          setError('Error en el formato de los datos recibidos');
          return;
        }

        console.log('Actividades cargadas:', eventosActividades);
    ...
    <div className="app-container">
      ...
      <main className="seccion-container">
        <div className="seccion-header">
          <h1 className="seccion-titulo">Actividades</h1>
          <p className="seccion-descripcion">Descubre las mejores actividades y experiencias</p>
        </div>
        ...
        <div className="eventos-grid">
          {eventos && eventos.length > 0 ? (
            ...
            <div className="no-eventos">
              ...
              <p className="no-eventos-sub">¡Vuelve pronto para ver nuevas actividades!</p>
            </div>
          )}
        </div>
