...
const Bienestar = () => {
  ...
  useEffect(() => {
    const cargarEventos = async () => {
      try {
        setLoading(true);
        console.log('Iniciando carga de eventos de bienestar...');
        const eventosBienestar = await getEventosPorCategoria("Bienestar");
        console.log('Respuesta del servidor:', eventosBienestar);
        ...
        if (!Array.isArray(eventosBienestar)) {
          console.error('La respuesta no es un array:', eventosBienestar);
          setError('Error en el formato de los datos recibidos');
          return;
        }

        console.log('Eventos de bienestar cargados:', eventosBienestar);
        setEventos(eventosBienestar);
      } catch (error) {
    ...
    return (
      <div className="app-container">
        ...
        <div className="loading-container">
          ...
          <p>Cargando eventos de bienestar...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="app-container">
      ...
      <main className="seccion-container">
        <div className="seccion-header">
          <h1 className="seccion-titulo">Bienestar</h1>
          <p className="seccion-descripcion">Descubre los mejores eventos de bienestar y experiencias</p>
        </div>
        ...
        <div className="eventos-grid">
          {eventos && eventos.length > 0 ? (
            ...
            <div className="no-eventos">
              <p>No hay eventos de bienestar disponibles en este momento</p>
              <p className="no-eventos-sub">¡Vuelve pronto para ver nuevos eventos de bienestar!</p>
            </div>
          )}
        </div>
