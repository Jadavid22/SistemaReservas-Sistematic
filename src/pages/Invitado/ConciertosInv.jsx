...
const ConciertosInv = () => {
  ...
  useEffect(() => {
    const cargarEventos = async () => {
      try {
        setLoading(true);
        console.log('Iniciando carga de eventos de conciertos...');
        const eventosConciertos = await getEventosPorCategoria("Conciertos");
        console.log('Respuesta del servidor:', eventosConciertos);
        
        if (!eventosConciertos) {
          console.warn('No se recibieron eventos del servidor');
          setEventos([]);
          return;
        }

        if (!Array.isArray(eventosConciertos)) {
          console.error('La respuesta no es un array:', eventosConciertos);
          setError('Error en el formato de los datos recibidos');
          return;
        }

        console.log('Eventos de conciertos cargados:', eventosConciertos);
        setEventos(eventosConciertos);
      } catch (error) {
    ...
    <div className="app-container">
      ...
      <main className="seccion-container">
        <div className="seccion-header">
          <h1 className="seccion-titulo">Conciertos</h1>
          <p className="seccion-descripcion">Descubre los mejores conciertos y eventos musicales</p>
        </div>
